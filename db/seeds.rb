...
def retrieve_nfts(wallet)
  base_url = 'https://api.opensea.io/api/v1'
  port_url = URI("#{base_url}/assets?owner=#{wallet}&order_direction=desc&offset=0&limit=20")
...
wallet_array.map do |wallet|
  retrieve_nfts(wallet)
end
